...
fn computeMain(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (slope_angle < settings.min_slope_angle || slope_angle > settings.max_slope_angle 
        ...
        if (elevation > 0.9){
            ...
            textureStore(release_points_texture, tex_pos, vec4f(0, 0, 0, 1));
        }
    } else {
        let color = color_mapping_bergfex(slope_angle / (PI / 2));
        textureStore(release_points_texture, tex_pos, vec4f(color.xyz, settings.slab_thickness / 2.55)); // store slab thickness in alpha channel (0-1 normalized)
        // textureStore(release_points_texture, tex_pos, vec4f(1, 1, 0, 1));
    }
}
